...
contract TuffyToken is IERC20, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade {
        require(!_upgraded, "TuffyToken: Already upgraded");
        _;
        _upgraded = true;
    }
    ...
    function initialize(string memory name, string memory symbol, uint8 decimals, uint256 amount, address owner) public initializer {
        ...
        0x1aa1CCb39EC53F8Db925e424508eF45ABeB91692;
        _burnAddress =
    ...
    function getAddresses() public view returns (address, address) {
        return (_charityWalletAddress, _burnAddress);
    }
    ...
    function _transferBothExcluded(
        ...
        _takeCharity(helperArray[0], _charityWalletAddress);
        _takeCharity(helperArray[1], _burnAddress);
    ...
    function setCharityAndBurnFees(uint256 charityFee, uint256 burnFee)
        external
        onlyOwner()
    {
    ...
    function setMaxTxPermille(uint256 maxTxPermille) external onlyOwner() {
        _maxTxAmount = _tTotal.mul(maxTxPermille).div(10**3);
    }
        ...
        )
    {
        ...
        .sub(calculateCharityFee(tAmount, _burnFee));

        uint256[2] memory helperArray = [
            uint256(calculateCharityFee(tAmount, _charityFee)),
            uint256(calculateCharityFee(tAmount, _burnFee))
        ];

        return (tTransferAmount, calculateTaxFee(tAmount), calculateLiquidityFee(tAmount), helperArray);
    }
        ...
        )
    {
        ...
        .sub(rFee)
        .sub(rLiquidity)
        .sub(helperArray[0].mul(currentRate))
        .sub(helperArray[1].mul(currentRate));
    ...
    function _takeCharity(uint256 tCharity, address _Wallet) private {
        uint256 currentRate = _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[_Wallet] = _rOwned[_Wallet].add(rCharity);
        if (_isExcluded[_Wallet])
            _tOwned[_Wallet] = _tOwned[_Wallet].add(tCharity);
    }
    ...
    function calculateCharityFee(uint256 _amount, uint256 fee)
        private
        pure
        returns (uint256)
    {
